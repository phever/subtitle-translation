...
def merge(video_file, original_srt, english_srt, original_lang):
    """
    ...
    cmd = [
        'ffmpeg', '-i', video_file, '-i', original_srt, '-i', english_srt,
        '-map', '0', '-map', '-0:s', '-map', '1', '-map', '2',
        '-c', 'copy',
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 5:
        print("Usage: python merge_subtitles.py <video_file> <original_srt> <english_srt> <original_lang>")
        sys.exit(1)
    
    video_file = sys.argv[1]
    original_srt = sys.argv[2]
    english_srt = sys.argv[3]
    original_lang = sys.argv[4]
    ...
    merge(video_file, original_srt, english_srt, original_lang)
